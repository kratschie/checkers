...
class Board
{
    ...
    private:
        ...
        bool can_jump_right_down(int zeile, int spalte);

				bool can_jump_left_down(int zeile, int spalte);
				
				bool can_go_right_up(int zeile, int spalte);
        
        bool can_go_left_up(int zeile, int spalte);
        
        bool can_jump_right_up(int zeile, int spalte);
...
bool Board::can_jump_right_down(int zeile, int spalte){
	return (((zeile < 6) && (spalte < 6)) && (field2d[zeile+1][spalte+1] == WHITE) && (field2d[zeile
	+2][spalte+2] == NONE)); 
}


bool Board::can_jump_left_down(int zeile, int spalte){
	return (((zeile < 6) && (spalte > 1)) && (field2d[zeile+1][spalte-1] == WHITE) && (field2d[zeile
	+2][spalte-2] == NONE));
}
